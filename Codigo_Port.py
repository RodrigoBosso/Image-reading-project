...
path = path.dirname(path.realpath(__file__)) + "\Imagenspor"
...
for file in files: # For que vai percorrer os Arquivos da pasta
  img = cv2.imread(f'Imagenspor/{file}') #img recebe cada arquivo por vez da pasta
  #Texto da imagem é lido Em portugues e enviado para a váriavel Texto
  tx = open("resultpor/img"+str(var)+".txt","w") # abrindo um txt para receber os resultados das imagens
...
#Traz o caminho do diretorio atual
path = path.dirname(path.realpath(__file__)) + "\\resultpor"
...
for file in files: # For que vai percorrer os Arquivos da pasta
    ...
    time.sleep(1)
    pyautogui.write(str(os.path.dirname(os.path.abspath(__file__))+"\\resultpor\\"))
    ...
    pyautogui.write(str(os.path.dirname(os.path.abspath(__file__))+"\\Audiopor\\"))
    pyautogui.press('enter')
    pyautogui.press('enter')
